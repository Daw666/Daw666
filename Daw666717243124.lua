...
pcall(function()
    if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().isfile(string.format("%s.daw", game.GameId)) and getfenv().readfile(string.format("%s.daw", game.GameId)) then
        ImportedConfiguration = HttpService:JSONDecode(getfenv().readfile(string.format("%s.daw", game.GameId)))
...
if typeof(script) == "Instance" and script:FindFirstChild("Fluent") then
    ...
    if Success and typeof(Result) == "string" and string.find(Result, "dawid") then
        ...
        Fluent = getfenv().loadstring(game:HttpGet("https://daw.pages.dev/Fluent.txt", true))()
    end
end
...
function InterfaceManager:ImportSettings()
    pcall(function()
        if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().isfile("UISettings.daw") and getfenv().readfile("UISettings.daw") then
            for Key, Value in next, HttpService:JSONDecode(getfenv().readfile("UISettings.daw")) do
...
function InterfaceManager:ExportSettings()
    pcall(function()
        if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().writefile then
            getfenv().writefile("UISettings.daw", HttpService:JSONEncode(UISettings))
        end
    end)
end
...
do
    ...
    if getfenv().isfile and getfenv().readfile and getfenv().writefile and getfenv().delfile then
        ...
        ConfigurationManager:AddButton({
            ...
            Callback = function()
                xpcall(function()
                    ...
                    getfenv().writefile(string.format("%s.daw", game.GameId), ExportedConfiguration)
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("Configuration File %s.daw has been successfully overwritten!", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
                    ...
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("An Error occurred when overwriting the Configuration File %s.daw", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
        ...
        ConfigurationManager:AddButton({
            ...
            Callback = function()
                if getfenv().isfile(string.format("%s.daw", game.GameId)) then
                    getfenv().delfile(string.format("%s.daw", game.GameId))
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("Configuration File %s.daw has been successfully deleted!", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
                    ...
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("Configuration File %s.daw could not be found!", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
...
local OnTeleport; OnTeleport = Player.OnTeleport:Connect(function()
    if DEBUG or not Fluent or not getfenv().queue_on_teleport then
        ...
        getfenv().queue_on_teleport("getfenv().loadstring(game:HttpGet(\"https://raw.githubusercontent.com/daw/Open-Aimbot/master/source.lua\", true))()")
        OnTeleport:Disconnect()
    end
end)
